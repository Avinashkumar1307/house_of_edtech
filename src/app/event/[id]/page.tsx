...
export default function PublicEventPage({ params }: PageProps) {
  ...
  useEffect(() => {
    fetchEvent();
  }, [resolvedParams.id]); // Use resolvedParams.id instead of params.id

  const fetchEvent = async () => {
    try {
      const response = await fetch(`/api/events/${resolvedParams.id}`); // Use resolvedParams.id
      if (response.ok) {
  ...
  const handleRsvpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/events/${resolvedParams.id}/rsvp`, { // Use resolvedParams.id
        method: "POST",
        headers: {
  ...
  const canRsvp = () => {
    ...
    if (event.maxAttendees && (event._count?.rsvps || 0) >= event.maxAttendees)
      return false;
    return true;
  };
        ...
        <AnimatePresence>
          {rsvpSuccess && (
            ...
            >
              ...
              <p className="text-gray-600 mb-6">
                Thank you for registering for {event.title}. You should receive
                ...
                {event.maxAttendees && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium">
                      {Math.max(
                        0,
                        event.maxAttendees - (event._count?.rsvps || 0)
                      )}
                    </span>
                  </div>
                )}
